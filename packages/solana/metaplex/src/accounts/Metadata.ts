...
export class Metadata extends Account<MetadataData> {
  ...
  static async findByOwner(connection: Connection, owner: AnyPublicKey) {
    ...
    const accountMap = new Map(
      accounts.map(({ data }) => [data.mint.toString(), data])
    );
    // Slow method
  ...
  static async findByOwnerV2(connection: Connection, owner: AnyPublicKey) {
    ...
    return (
      await Promise.all(
        accountsWithAmount.map(({ mint }) =>
          Metadata.findMany(connection, { mint })
        )
      )
    ).flat();
  }

  static async findByOwnerV3(
    connection: Connection,
    owner: AnyPublicKey
  ): Promise<Metadata[]> {
    const tokenInfo = await Metadata.findInfoByOwner(connection, owner);

    return Array.from(tokenInfo.entries()).map(
      ([pubkey, info]) => new Metadata(pubkey, info)
    );
  }
  ...
  static async findDataByOwner(
    ...
    return Array.from(tokenInfo.values()).map((m) =>
      MetadataData.deserialize(m.data)
    );
  }
